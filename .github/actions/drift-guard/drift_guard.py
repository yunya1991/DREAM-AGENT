...
def _match_module(modules: Dict[str, Any], file_path: str) -> Optional[str]:
    ...
    for name in sorted(modules.keys()):
        mod_cfg = modules.get(name) or {}
        if not isinstance(mod_cfg, dict):
            continue
        patterns = mod_cfg.get("paths") or []
...
def _write_github_output(
    *,
    ...
    with open(github_output, "a", encoding="utf-8") as f:
        ...
        f.write(f"reason_codes={json.dumps(reason_codes, ensure_ascii=False)}\n")
        f.write(f"report_json_path={report_json_path}\n")
        f.write(f"report_md_path={report_md_path}\n")
